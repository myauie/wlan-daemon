...
int 
open_socket(int domain) {
        return socket(domain, SOCK_DGRAM, 0);
}

const char     *
mediatype(char *interface) {
...
#define tohex(x)        (isdigit(x) ? (x) - '0' : tolower(x) - 'a' + 10)
                *p++ = (tohex((u_char) val[0]) << 4) | tohex((u_char) val[1]);
#undef tohex
                val += 2;
            } else {
...
/* Code from ifconfig. */
void 
set_network_id(char *network_ssid, char *if_name) {
        int 		s = -1, 	res, size;
        struct ifreq 	ifr;
...
int 
set_wep_key(char *wep_key, char *if_name, int toggle) {
        int 		s = -1, 	res, len;
        ...
        if (toggle) {
            ...
            /*
             *  For 40-bit WEP, length of key must be 5 char ASCII string or 10 hex digits.
             *  For 128-bit WEP, length of key must be 13 char ASCII string or 26 hex digits.
             */
            if ((strlen(wep_key) == 5) || (strlen(wep_key) == 13))
                /* String case. */            
                printf("wep key is correct length\n");
            else if ((strlen(wep_key) == 12) || (strlen(wep_key) == 28)) {
                /* 0x key case, hex should be + 2 to include 0x. */
                printf("wep key is correct length\n");
...
int 
set_psk_key(char *nwid, char *psk_key, char *if_name, int toggle) {
        int 		s = -1, 	res, size;
        struct ieee80211_wpapsk psk;
        struct ieee80211_wpaparams wpa;
        struct ifreq 	ifr;

        printf("setting psk key\n");
        s = open_socket(AF_INET);
        if (!s)
            printf("socket error: %s\n", strerror(errno));
...
void 
set_bssid(char *network_bssid, char *if_name, int toggle) {
        int 		s = -1, 	res;
...
void 
set_wpa8021x(char *if_name, int toggle) {
        int 		res      , s = -1;
        struct ieee80211_wpaparams wpa;
        struct ifreq 	ifr;
